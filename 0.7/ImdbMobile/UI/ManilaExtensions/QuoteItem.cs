    ...
    class QuoteItem : MichyPrima.ManilaDotNetSDK.KListControl.IKListItem
    {
        ...
        public void CalculateHeight()
        {
            ...
            using (Graphics g = Graphics.FromImage(b))
            {
                ...
                {
                    g.FillRectangle(new SolidBrush(IMDBData.SettingsWrapper.GlobalSettings.CurrentSkin.BackgroundColour), CurrentBounds);
                }
                if (this.Icon != null)
                {
                    Size s = Extensions.GetBitmapDimensions(this.Icon);
                    Rectangle DestRect = new Rectangle(5, PaddingTop, s.Width, s.Height);
